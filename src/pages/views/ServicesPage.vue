<template>
    ...
    <div class="card">
      ...
      >
        ...
        <Column
          ...
          style="width: 20%"
        ></Column>
        <Column field="currency" header="Currency" style="width: 20%"></Column>
        <Column field="price" header="Cost/Price" style="width: 20%"></Column>
  ...
  <AddNewService v-if="modal_add" @close="modal_add = false" />
  <UpdateService v-if="modal_update" @close="modal_update = false" :data="selectedData" />
...
const deleleData = async () => {
  const loadingInstance = ElLoading.service({
    lock: true,
    text: "Deleting Services's Data...",
  });
  if (!selectedData.value) return;
  const docRef = doc(db, "services", selectedData.value.id);
