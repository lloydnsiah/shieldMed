    ...
    <section class="px-8 py-10 relative">
      ...
      <div class="flex flex-col">
        <div class="flex flex-col">
          ...
          <span v-if="props.data.additionalCharges.length === 0" class="text-sm text-gray-700">
      ...
      <div class="flex justify-between text-md font-bold text-gray-500">
        <span>Grand Total:</span> <span>GH₵ {{ props.data.grandtotal.toFixed(2) }}</span>
      </div>
      <Divider />
      <div class="grid grid-cols-2 gap-2">
        <Button label="Cancel" severity="secondary" @click="emit('close')" />
        <Button label="Print" @click="onSubmit" />
      </div>
    </section>
  </main>
</template>
